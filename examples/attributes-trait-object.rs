...
trait BufferData {
    fn to_gl_buffer(&self) -> i32;
}
impl BufferData for Vec<f32> {
    fn to_gl_buffer(&self) -> i32 {
        println!("Convert and upload {:?}", *self);
        1
    }
}
impl BufferData for Vec<[f32; 2]> {
    fn to_gl_buffer(&self) -> i32 {
        println!("Convert and upload {:?}", *self);
        2
    }
}
impl BufferData for Vec<[f32; 3]> {
    fn to_gl_buffer(&self) -> i32 {
        println!("Convert and upload {:?}", *self);
        3
    }
}
impl BufferData for Vec<[f32; 4]> {
    fn to_gl_buffer(&self) -> i32 {
        println!("Convert and upload {:?}", *self);
        4
    }
}
impl BufferData for Vec<[f32; 9]> {
    fn to_gl_buffer(&self) -> i32 {
        println!("Convert and upload {:?}", *self);
        5
    }
}
impl BufferData for Vec<[f32; 16]> {
    fn to_gl_buffer(&self) -> i32 {
        println!("Convert and upload {:?}", *self);
        6
    }
}
impl BufferData for i32 {
    fn to_gl_buffer(&self) -> i32 {
        println!("Convert and upload {:?}", *self);
        7
    }
...
fn main() {
    ...
    for &(ref name, ref data) in attributes.list.iter() {
        println!(
            "\nI sent this attribute to GL-land: {:?}, and got back {:?}",
            name, data
        );
    }
}
